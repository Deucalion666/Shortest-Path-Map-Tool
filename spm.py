...
def write_pymol(outpath, pdb_obj, edges, node_scores):
    with open(outpath,"w") as f:
        ...
        edge_objs=[]
        nscores=np.array(list(node_scores.values()))
        lo,hi=nscores.min(),nscores.max() if nscores.size else (0,1)
        def scale_node(s): return 0.4+0.7*(s-lo)/(hi-lo+1e-12)
        escores=[s for (_,_,s,_) in edges]
        emin,emax=min(escores),max(escores) if escores else (0,1)
        def scale_edge(s): return 0.4+0.7*(s-emin)/(emax-emin+1e-12)
        for idx,(i,j,s,corr) in enumerate(edges, start=1):
            ...
            f.write(f"set sphere_scale,{scale_node(node_scores[i]):.6f}, {obj} and resi {i}\n")
            f.write(f"set sphere_scale,{scale_node(node_scores[j]):.6f}, {obj} and resi {j}\n")
            f.write(f"bond name ca and resi {i}, name ca and resi {j}\n")
            f.write(f"set stick_radius,{scale_edge(s):.6f}, {obj}\n")
...
def main():
    ...
    ap.add_argument("-s","--significance",type=float,default=0.3,help="Significance cutoff (0–1)")
